...
def load_camera_parameters(reader_directory, scan_path, scan, unzipped_directory):
    sens_file = os.path.join(scan_path, scan + ".sens")
    output_directory = os.path.join(unzipped_directory, scan)
...
def get_original_frame(unzipped_directory, frame_index):
    original_path = os.path.join(unzipped_directory, "color", f"{frame_index}.jpg")
...
def process_frame(unzipped_directory, frame_index, vis, camera_intrinsics, instance_mesh, error):
    original_frame = get_original_frame(unzipped_directory=unzipped_directory, frame_index=frame_index)
    if original_frame is None: return
    image_height, image_width = original_frame.shape[:2]
    camera_extrinsics = get_camera_extrinsics(frame_index, unzipped_directory)
...
def get_masked_images(scene_unzipped, masked_object_directory, scene_mesh, frame_indices, camera_intrinsics, instance_mesh, error):
    if not os.path.exists(masked_object_directory):
        os.makedirs(masked_object_directory)
    original_frame = get_original_frame(scene_unzipped, 0)
    image_height, image_width = original_frame.shape[:2]
    ...
    for frame_index in tqdm(sorted(frame_indices), total=total_frames, desc="Processing Frames"):
        masked_frame = process_frame(unzipped_directory=scene_unzipped, frame_index=frame_index, vis=scene_vis, camera_intrinsics=camera_intrinsics, instance_mesh=instance_mesh, error=error)
...
reader_directory = "/Users/lara/Desktop/Making-CLIP-features-multiview-consistent/scannet_scripts/SensReader"
...
objects_to_exlude = {"wall", "floor", "closet floor", "door wall"}
...
for scan in scans:
    ...
    if scan.endswith("_00"):
        ...
        total_seg_groups = len(aggregation_data['segGroups'])
        for seg_group in tqdm(aggregation_data['segGroups'], total=total_seg_groups, desc="Processing SegGroups"):
            if seg_group['label'] in objects_to_exlude:
                continue
            ...
            camera_intrinsics, frame_indices = load_camera_parameters(reader_directory=reader_directory, scan_path=scan_path, scan=scan, unzipped_directory=unzipped_directory)
            minimum_vertices = get_minimum_vertices(total_vertices=instance_vertices.shape[0], visible_object=visible_object)
            get_masked_images(scene_unzipped=os.path.join(unzipped_directory, scan), 
                            masked_object_directory=os.path.join(masked_scannet_directory, "cropped_images/project_without_occlusion", scan, f"{object_id}"),
